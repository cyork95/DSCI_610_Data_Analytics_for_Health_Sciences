...
heat.map <- ggplot(data = melted_corr_mat, aes(x=Var1, y=Var2,fill=value)) +
  ...
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red")+
  labs(fill="Correlation", title = "Figure 1: Correlation Heat Map")+
  geom_tile() +
...
error_rate <- c(sub_glm_error_rate,sub_tree_error_rate,sub_rf.parkinsons$err.rate[500,1],svm_error_rate)
...
# Grip Plot
grid.arrange(plot2,plot3,plot4,plot1)
```
...
is created by subtracting the mean of the amount of correct observations from 1. This gives us a percentage of the time the model make an incorrect prediction. To create the **confusion matrix** we use the **kable()** method to create a table of the amount of
...
To select the features to be used in the models, we first examined the correlation matrix from all 22 features(Figure 1). Since the features all came from biomedical voice measurements, some of the features were likely to be heavily correlated, and thus redundant. Looking at the correlation heat map in figure 1, there are a large number of the features that are significantly correlated. Using the findCorrelation() function we identified the features with an absolute correlation over 0.5, and excluded them from our feature list. This left us with the 6 features that we used for our models. Getting rid of these redundant features improves the models accuracy, and reduces over-fitting. 
...
|               Figure 2: Confusion matrices of the Logistic Regression, Decision Tree, Random Forest, and SVM models.
...
## Conclusion
In conclusion, the dataset used in this study was effective in determining if a person had Parkinson's disease.
The random forest model was able to achieve an accuracy of 92%, which is significantly higher than the accuracy of other
methods that have been used to diagnose Parkinson's disease. This suggests that the random forest model could be a valuable
tool for early diagnosis of Parkinson's disease assuming you can get measurements on vocal changes, shimmer and jitter.

This study was limited to the size of the dataset available which as discussed only contained 197 attributes. This study could
be more thorough if we had more instances to work with. Another limitation would be that we didn't have many demographic or
other health vitals to work with. Some of those features could also be very helpful in making determinations.

Overall, the results of this study suggest that the random forest model is a promising tool for early diagnosis of Parkinson's disease.
Future studies should use larger datasets with a bigger variety of features to confirm the findings of this study and to
explore the use of the random forest model for other neurological disorders.
