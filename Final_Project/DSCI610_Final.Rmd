...
setwd("C:\\Users\\cy_su\\DataspellProjects\\DSCI_610_Data_Analytics_for_Health_Sciences\\Final_Project")
```
```{r}
...
```
```{r}
lm.parkinsons.tr <- glm(status ~ avg_vocal_freq + percent_measure_of_jitter + ddp_measure_of_jitter +
                                rap_measure_of_vocals + ppq_measure_of_vocals + measure_of_shimmer +
...
```
```{r}
tree.parkinsons.tr <- tree(status ~ avg_vocal_freq + percent_measure_of_jitter + ddp_measure_of_jitter +
                            rap_measure_of_vocals + ppq_measure_of_vocals + measure_of_shimmer +
...
```
```{r}
rf.parkinsons <- randomForest(status ~ avg_vocal_freq + percent_measure_of_jitter + ddp_measure_of_jitter +
                                rap_measure_of_vocals + ppq_measure_of_vocals + measure_of_shimmer +
                                apq_measure_of_shimmer + dda_measure_of_shimmer + hnr, data = predict_parkinsons.train)
rf.parkinsons
